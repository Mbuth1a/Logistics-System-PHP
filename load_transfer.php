...
$sql = "SELECT 
        ...
        FROM transfers
        LEFT JOIN load_transfer ON transfers.transfer_id = load_transfer.id
        LEFT JOIN drivers ON transfers.driver = drivers.id  /* Join with the drivers table */
        LEFT JOIN vehicles ON transfers.vehicle = vehicles.id  /* Join with the vehicles table */
        WHERE load_transfer.id IS NULL"; /* Filter out trips already in load_export table */
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        // Create a display string with driver and vehicle information
        $tripDisplay = $row['transfer_id'] . ', ' . 
                       $row['customer_name'] . ', ' . 
                       $row['transfer_date'] . ', ' . 
                       $row['transfer_time'] . ', ' . 
                       $row['transfer_day'] . ', ' . 
                       $row['driver'] . ', ' .  // Use the driver's name
                       $row['vehicle'] . ', ' . // Use the vehicle's registration number
                       $row['destination'];
        // Generate the option tag
        $tripOptions .= '<option value="' . $row['transfer_id'] . '" data-trip-date="' . $row['transfer_date'] . '" data-customer-name="' . $row['customer_name'] . '">' . $tripDisplay . '</option>';
    }
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($transfer_id && $total_weight && count($products) > 0) {
        // Insert into load_trip table
        $insertTripLoad = $conn->prepare("INSERT INTO load_transfer (transfer_id, total_weight) VALUES (?, ?)");
        $insertTripLoad->bind_param("id", $transfer_id, $total_weight);

        if ($insertTripLoad->execute()) {
            $tripLoadId = $insertTripLoad->insert_id;

            // Insert products into load_trip_products table
            $insertProduct = $conn->prepare("INSERT INTO load_transfer_products (load_transfer_id, product_description, quantity, total_weight) VALUES (?, ?, ?, ?)");
            ...
            $insertProduct->close();
             ...
             header("Location: load_transfer.php?success=1");
             exit();
        } else {
...
<head>
    ...
    <title>Load Transfer Trip</title>
...
<body>
    ...
    <div class="container-fluid main-content col-md-10">
        <h2 class="text-center text-orange"><i class="fas fa-box"></i> Load Transfer Trip</h2>
        ...
        <form id="loadTripForm" method="post" action="load_transfer.php">
            <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
            <div class="form-group">
                <label for="id_trip"><i class="fas fa-cube"></i> Trip</label>
                <select id="id_trip" name="transfer_id" class="form-control" onchange="tripSelected()">
