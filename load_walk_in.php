...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $tripDisplay = $row['walk_id'] . ', ' . 
               $row['walk_trip_day'] . ', ' . 
               $row['walk_trip_time'] . ', ' . 
               $row['walk_trip_date'];
        $tripOptions .= '<option value="' . $row['walk_id'] . '" data-trip-date="' . $row['walk_trip_date'] . '">' . $tripDisplay . '</option>';
    }
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($walk_id && $total_weight && count($products) > 0) {
        // Insert into load_trip table
        $insertTripLoad = $conn->prepare("INSERT INTO load_walk_in (walk_in_id, total_weight) VALUES (?, ?)");
        $insertTripLoad->bind_param("id", $walk_id, $total_weight);

        if ($insertTripLoad->execute()) {
            $tripLoadId = $insertTripLoad->insert_id;

            // Insert products into load_trip_products table
            $insertProduct = $conn->prepare("INSERT INTO load_walk_in_products (load_walk_in_id, product_description, quantity, total_weight) VALUES (?, ?, ?, ?)");
            ...
            $insertProduct->close();
             ...
             header("Location: load_walk_in.php?success=1");
             exit();
        } else {
...
<body>
    ...
    <div class="container-fluid main-content col-md-10">
        <h2 class="text-center text-orange"><i class="fas fa-box"></i> Load Walk In Trip</h2>
        ...
        <form id="loadTripForm" method="post" action="load_walk_in.php">
            <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
            <div class="form-group">
                <label for="id_trip"><i class="fas fa-cube"></i> Trip</label>
                <select id="id_trip" name="walk_id" class="form-control" onchange="tripSelected()">
    ...
    <script src="js/load_walk_in.js"></script>
